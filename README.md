...
| 이름             | HTTP 메서드 | 경로          | HEADER                          | Request Body                    |
|------------------|-------------|-------------|---------------------------------|---------------------------------|
| 포인트 조회      | GET         | /api/points | Authorization: Bearer {토큰}     | 없음                            |
| 포인트 충전      | POST        | api/points  | Content-Type: application/json   | point: {정수(필수)}              |

### Response (성공 시)

| 이름             | 상태 코드 | 경로          | 헤더                          | Response Body         |
|------------------|-----------|-------------|--------------------------------|-----------------------|
| 포인트 조회 성공 | 200       | /api/points | Authorization: Bearer {토큰}   | point: {정수}          |
| 포인트 충전 성공 | 200       | /api/points | Authorization: Bearer {토큰}   | point: {정수}          |

### Response (실패 시)

| 이름               | 경로     | 상태 코드 | 예외 사항                       | Response Body |
|--------------------|----------|-----------|--------------------------------|---------------|
| 포인트 조회 실패  | /points  | 401       | 인증되지 않은 접근              | -             |
| 포인트 조회 실패  | /points  | 500       | 서버 내부 오류                  | -             |
| 포인트 충전 실패  | /points  | 400       | 잘못된 요청 (포인트 값이 잘못됨) | -             |
| 포인트 충전 실패  | /points  | 401       | 인증되지 않은 접근              | -             |
| 포인트 충전 실패  | /points  | 500       | 서버 내부 오류                  | -             |
