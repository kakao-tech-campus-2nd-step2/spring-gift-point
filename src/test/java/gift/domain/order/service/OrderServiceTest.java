...
class OrderServiceTest {
    ...
    private static final User user = new User(1L, "testUser", "test@test.com", "test123", Role.USER, AuthProvider.KAKAO);
    ...
    void createAndSendMessage() {
        ...
        OrderResponse expected = OrderResponse.from(orderRequest.toOrder(user));

        doNothing().when(orderItemService).create(any(User.class), any(Order.class), any(List.class));
        given(orderJpaRepository.save(any(Order.class))).willReturn(order);
        given(messageService.sendMessageToMe(any(User.class), any(OrderResponse.class))).willReturn("0");

        // when
        OrderResponse actual = orderService.createAndSendMessage(orderRequest, user);
