...
public class UserServiceTest {
    ...
    void test1(){
        ...
        UserRequest userRequest = new UserRequest( "user1@email.com", "aaaaa");
    ...
    void test2(){
        ...
        for(int i = 1; i <= 5; i++){
            User user = new User( "user" + i + "@email.com", "" + i);
            users.add(user);
        }
        ...
        // when
        Page<UserResponse> pageResult = userService.findAll(0, 5, "name", "asc");
        // then
        ...
        for(int i = 1; i <= 5; i++){
            String email = content.get(i-1).getEmail();
            Assertions.assertThat(email).isEqualTo("user"+i+"@email.com");
        }
    }
    ...
    void test3(){
        ...
        given(userRepository.findAll(pageable)).willAnswer(invocation -> page);
        // when
        Page<UserResponse> pageResult = userService.findAll(0, 5, "name", "desc");
        // then
        Assertions.assertThat(pageResult).isNotNull();
        Assertions.assertThat(pageResult.get().count()).isEqualTo(5);
        List<UserResponse> content = pageResult.getContent();
    ...
    void test4(){
        ...
        given(userRepository.findByEmail("user1@email.com")).willAnswer(invocation -> user);
        // when
        UserResponse userResponse = userService.findByEmail("user1@email.com");
        // given
    ...
    void test6(){
        // given
        UserRequest userRequest = new UserRequest( "user2@email.com", "aaaab");
