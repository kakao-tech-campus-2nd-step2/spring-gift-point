...
public class WishRestController {
    ...
    @PostMapping()
    public void create(@RequestAttribute("Email") String email, @RequestParam String name){
    ...
    public void delete(@RequestAttribute("Email") String email, @PathVariable Long wishId){
        wishService.delete(email, wishId);
    }

    @Operation(summary = "위시리스트 조회", description = "email을 받아 wish 데이터베이스에 있는 데이터를 조회한다.")
    @GetMapping()
    public Page<String> readPage(@RequestAttribute("Email") String email, @RequestParam(value = "page", defaultValue = "0") int page,
