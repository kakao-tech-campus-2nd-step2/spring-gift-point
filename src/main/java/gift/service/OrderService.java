...
public class OrderService {
    ...
    public OrderResponse createOrder(String token, OrderRequest orderRequest) {
        ...
        // 포인트 사용
        int orderPrice = option.getQuantity() * option.getProduct().getPrice();
        if(orderPrice > user.getPoint()) {// 가격보다 사용자의 포인트가 적다면
            user.subtractPoints(user.getPoint()); // 사용자의 포인트 전액 사용
            userRepository.save(user);
            orderPrice -= user.getPoint();
        }
        if (orderPrice <= user.getPoint()) { // 가격보다 사용자의 포인트가 크다면
            user.subtractPoints(orderPrice); // 가격만큼의 사용자 포인트 사용
            userRepository.save(user);
            orderPrice = 0;
        }
        ...
        return OrderResponse.from(order, orderPrice);
    }
