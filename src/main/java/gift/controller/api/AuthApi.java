...
public interface AuthApi {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "409", description = "회원 가입 실패(사유 : 이미 존재하는 이메일입니다. )", content = @Content(schema = @Schema(hidden = true))),
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "회원 등록 실패(사유 : 이메일이나 비밀번호가 올바르지 않습니다. )", content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "500", description = "내부 서버의 오류", content = @Content(schema = @Schema(hidden = true)))
    })
